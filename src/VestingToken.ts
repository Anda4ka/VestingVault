/**
 * VestingToken — Simple mintable OP20 test token
 * Used as the "vesting token" for VestingVault testing on OPNet testnet.
 *
...
@final
export class VestingToken extends OP20 {
    public constructor() {
        super();
    }
    ...
    public override onDeployment(_calldata: Calldata): void {
        ...
        this.instantiate(new OP20InitParameters(maxSupply, 18, 'Vesting Token', 'VEST'));
    }
    ...
    public mint(calldata: Calldata): BytesWriter {
        ...
        if (to.equals(Address.zero())) {
            throw new Revert('VestingToken: mint to zero address');
        }
        if (amount.isZero()) {
            throw new Revert('VestingToken: mint amount is zero');
        }
